...
public class FPTree {
  ...
  private int[] attribute;//树节点对应的item编号
  private int[] childCount;//树节点的子节点个数
  ...
  private int[][] headerTableProperties;//存储每个item 在树中对应的系列节点，store node links
  private int[] next;//headerTableProperties和next一起存储node links, 不同item的node link 放在一条线上就形成一个线性数组
  private int[][] nodeChildren;
  private long[] nodeCount;//此树中item出现的次数
  private int nodes;//node 的索引
  private int[] parent;
  ...
  public final void addHeaderCount(int attributeValue, long count) {
    int index = getHeaderIndex(attributeValue);//index is the item position in headTable
  ...
  //childAttribute item 的编号
  //如果nodeid的子节点item index= childAttribute,返回 该子节点，否则返回-1
